...
class DetectionNN {
    ...
    public:
         ...
         * @param frames original frame to draw bounding box on.
         */
        void draw(std::vector<cv::Mat>& frames, std::vector<std::vector<int>> &detret, std::vector<std::string> &classesnames) {
            tk::dnn::box b;
            int x0, w, x1, y0, h, y1;
            int objClass;
            std::string det_class;
            ...
            for(int bi=0; bi<frames.size(); ++bi){
                ...
                for(int i=0; i<batchDetected[bi].size(); i++) { 
                    ...
                    retInOneFrame.push_back(b.x);
                    retInOneFrame.push_back(b.y);
                    retInOneFrame.push_back(b.w);
                    retInOneFrame.push_back(b.h);
                    retInOneFrame.push_back(b.cl);
                    retInOneFrame.push_back(b.prob*100);
                    classesnames = classesNames;
                    // b.print();
                    // draw rectangle
                    // if(b.cl >= 4)
