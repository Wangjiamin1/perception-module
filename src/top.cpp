...
Top::Top(){
    ...
    init_rect[0] = 640 - AUTO_TRACK_SIZE_M/2;
    init_rect[1] = 360 - AUTO_TRACK_SIZE_M/2;
    init_rect[2] = init_rect[3] = AUTO_TRACK_SIZE_M;
...
void Top::decode_tcp_data(){
  ...
  printf("rec[3]:%d, rec[5]:%d, dispmode=%d, ctrlMode=%d\n",
  rec_param[3], rec_param[5], m_enDispMode, m_enCtrlMode);
}
...
int Top::run(){
  ...
  while(!quit)
  {
    ...
    // if(mode_frame == 1 && mode_fun == 1){
      ...
      // RTSP->process(thermalImg);
    // }
    // else if(mode_frame == 2 && mode_fun == 1){
    //   // visCam->getFrame(thermalImg);
      //  RTSP->process(visImg);
    // }
